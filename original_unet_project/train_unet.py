...
def train_one_epoch(
    ...
    for images, masks, _ in pbar:
        ...
        dice, iou, precision, recall = binary_metrics(logits.detach(), masks)
    ...
    return Metrics(
        ...
        precision=total_precision / total_items,
        recall=total_recall / total_items,
    )
...
def evaluate(
    ...
    for images, masks, _ in pbar:
        ...
        batch_size = images.size(0)
        dice, iou, precision, recall = binary_metrics(
            metric_logits,
            masks,
            threshold=threshold,
...
def find_best_threshold(
    ...
    for images, masks, _ in pbar:
        ...
        for values in totals:
            ...
            values["precision"] = float(values["precision"]) + precision * batch_size
            values["recall"] = float(values["recall"]) + recall * batch_size
            values["items"] = int(values["items"]) + batch_size

    rows = []
    best = ThresholdMetrics(threshold=0.5, dice=-1.0, iou=-1.0, precision=-1.0, recall=-1.0, postprocess=None)
    for values in totals:
        items = max(int(values["items"]), 1)
        dice = float(values["dice"]) / items
        iou = float(values["iou"]) / items
        precision = float(values["precision"]) / items
        recall = float(values["recall"]) / items
        threshold = float(values["threshold"])
...
def plot_training_curves(history_path: Path, output_path: Path) -> None:
    ...
    plots: list[tuple[str, str, str, str]] = [
        ...
        ("Precision", "train_precision", "val_precision", "higher is better"),
        ("Recall", "train_recall", "val_recall", "higher is better"),
    ]
    plots = [
        plot
        for plot in plots
        if all(plot[1] in row and plot[2] in row for row in history)
    ]
    if not plots:
        return
    ...
    width, height = 1500, 120 + rows * 360
    margin = 46
    panel_gap = 34
    row_gap = 36
    panel_width = (width - margin * 2 - panel_gap * (columns - 1)) // columns
    panel_height = 300
    top = 64
    ...
    for index, (title, train_key, val_key, ylabel) in enumerate(plots):
        ...
        left = margin + column_index * (panel_width + panel_gap)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="Dataset root folder.")
    ...
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_RUNS_DIR / "unet")
    ...
    if args.eval_only:
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            row = {
                ...
                "val_best_precision": epoch_threshold_metrics.precision,
                "val_best_recall": epoch_threshold_metrics.recall,
            }
